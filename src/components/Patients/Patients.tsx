  ...
  const fetchPatients = async () => {
    ...
    try {
      ...
      const response = await httpService.get<ApiResponse>(
        `admin/patients?page=${currentPage}&per_page=${itemsPerPage}&sort_by=${sortBy}&sort_dir=${sortDir}&search=${debouncedSearchTerm}`,
        {
          headers: {
                ...
                {sortedPatients.length > 0 ? (
                  sortedPatients.map((patient) => (
                    <tr
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/patients/${encryptId(patient.id)}`, { state: { patient } });
                      }}
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center">
                          ...
                          <div className="ml-3">
                            <button
                              onClick={() => navigate(`/patients/${encryptId(patient.id)}`, { state: { patient } })}
                      ...
                      <td className="px-4 py-3">
                        <div className="text-xs text-gray-600 space-y-1">
                          ...
                          <div>
                            ...
                            <span className="ml-2">{calculateAge(patient.date_of_birth)}</span>
                          </div>
                          <div>
                      ...
                      <td className="px-4 py-3">
                        <div className="text-sm text-gray-900">
                          <div className="font-medium">{formatDate(patient.last_appointment_date || patient.created_at)}</div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
